// - add a popMatrix to the draw loop - with momentary switch
...
void paint() { // draw the currently populated array
  for (int i = 0; i < strokes.size(); i++) { // cycle through each index of the strokes arrayList
    ...
    for (int r = 0; r < recur; r++) {
      stroke.set_size(brush_size+i); // set_size of the current stroke
      stroke.set_translateX(translateX);
      stroke.set_translateY(translateY);
