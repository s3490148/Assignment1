...
class Brush {
  ...
  void draw() {
    imageMode(CENTER);
    for (int r = 0; r < img_recur; r++) { // for each stroke drawn * number of recurences
      rotate(r*img_rotate);
      translate(img_translate,img_translate);
      pushMatrix();
      translate(x + img_translateX, y+ img_translateY + r);
